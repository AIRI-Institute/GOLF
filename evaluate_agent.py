...
def eval_agent(env, actor, n_confs, evaluate_rdkit, evaluate_rl, eval_termination_mode):
    ...
    for _ in tqdm(range(n_confs)):
        ...
        _, final_rdkit_energy = rdkit_minimize_until_convergence(
        ...
        if evaluate_rl:
            ...
            _, final_rdkit_after_rl_energy = rdkit_minimize_until_convergence(
                env, after_rl_atoms, smiles, M=0
            )
        ...
        for n_its in range(1, 1000):
            ...
            if not rdkit_converged_flag and evaluate_rdkit:
                ...
                result[f"{n_its}"]["rdkit"].append(
                    (initial_energy - final_energy_rdkit) / total_delta_rdkit
                )
                if not nc:
            ...
            if not rdkit_after_rl_converged_flag and evaluate_rl:
                final_energy_rdkit_after_rl, nc_after_rl = rdkit_minimize(
                    env, after_rl_atoms, smiles, n_its
                )
                result[f"{n_its}"]["rdkit_after_rl"].append(
                    (initial_energy - final_energy_rdkit_after_rl)
                    / total_delta_rdkit_after_rl
                )
                result[f"{n_its}"]["rdkit_after_rl_rel"].append(
                    (initial_energy - final_energy_rdkit_after_rl) / total_delta_rdkit
                )
                if not nc_after_rl:
                    ...
                    for i in range(n_its + 1, max_iter + 1):
                        ...
                        result[f"{i}"]["rdkit_after_rl_rel"].append(
                            (initial_energy - final_energy_rdkit_after_rl)
                            / total_delta_rdkit
                        )
...
def main(checkpoint_path, args, config):
    ...
    result, convergence_results = eval_agent(
        eval_env,
        actor,
        args.conf_number,
        args.evaluate_rdkit,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        metavar="True|False",
        type=str2bool,
        help="Evaluate initial state with rdkit",
    )
    parser.add_argument(
        "--evaluate_rl",
        default=False,
        ...
        metavar="True|False",
        type=str2bool,
        help="Evaluate after-RL state with rdkit",
    )
