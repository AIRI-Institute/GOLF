...
def main(checkpoint_path, args, config):
    ...
    while not np.all(finished).item():
        ...
        not_finite_action_energies[non_finite_action_mask] = energies_ground_truth[
            non_finite_action_mask
        ]
        ...
        for threshold in convergence_thresholds:
            ...
            convergence_info[threshold]["convergence_step"][convergence_mask] = steps[
                convergence_mask
            ]
            convergence_info[threshold]["convergence_n_iter"][
                convergence_mask
            ] = n_iters[convergence_mask]
            convergence_info[threshold]["convergence_energy"][
                convergence_mask
            ] = energies_ground_truth[convergence_mask]
        ...
        for early_stop_step in early_stop_steps:
            ...
            for i in np.where(
                (np.isinf(energies_ground_truth) | (forces_ground_truth == None))
                & early_stop_step_mask
            )[0]:
                (
                    _,
                    energies_ground_truth[i],
                    forces_ground_truth[i],
                ) = eval_env.minimize_rdkit(i)

            early_stop_info[early_stop_step]["early_stop_energy"][
                early_stop_step_mask
            ] = energies_ground_truth[early_stop_step_mask]
            early_stop_info[early_stop_step]["early_stop_n_iter"][
                early_stop_step_mask
            ] = n_iters[early_stop_step_mask]
            early_stop_info[early_stop_step]["early_stop_flag"][
                early_stop_step_mask
            ] = 1
        ...
        if eval_env.minimize_on_every_step:
            ...
            negative_reward_n_iters[first_negative_reward_mask] = n_iters[
                first_negative_reward_mask
            ]
            ...
            with open(args.evaluation_metrics_path, "a") as file_obj:
                if not finished[i] and done:
                    ...
                    (
                        initial_energy,
                        final_energy_rdkit,
                    ) = rdkit_minimize_until_convergence(
                        initial_env, [molecules[i]], [smiles[i]], max_its=0
                    )
                    stats["rdkit_initial_energy"].append(float(initial_energy))
                    stats["rdkit_final_energy"].append(float(final_energy_rdkit))

                    pct = (initial_energy - info["final_energy"][i]) / (
                    ...
                    stats["pct_of_minized_energy_lbfgs_done"].append(
                        (initial_energy - lbfgs_done_energy)
                        / (initial_energy - final_energy_rdkit)
                    )
                    ...
                    for threshold in convergence_thresholds:
                        convergence_step = convergence_info[threshold][
                            "convergence_step"
                        ][i]
                        convergence_n_iter = convergence_info[threshold][
                            "convergence_n_iter"
                        ][i]
                        convergence_energy = convergence_info[threshold][
                            "convergence_energy"
                        ][i]
                        convergence_flag = convergence_info[threshold][
                            "convergence_flag"
                        ][i]
                        ...
                        stats[f"convergence_n_iter@thresh:{threshold}"].append(
                            int(convergence_n_iter)
                        )
                        stats[f"converged@thresh:{threshold}"].append(
                    ...
                    for early_stop_step in early_stop_steps:
                        early_stop_energy = early_stop_info[early_stop_step][
                            "early_stop_energy"
                        ][i]
                        early_stop_flag = early_stop_info[early_stop_step][
                            "early_stop_flag"
                        ][i]
                        early_stop_n_iter = early_stop_info[early_stop_step][
                            "early_stop_n_iter"
                        ][i]
                        energy_mse = early_stop_info[early_stop_step]["energy_mse"][i]
                        force_mse = early_stop_info[early_stop_step]["force_mse"][i]
                        force_norm = early_stop_info[early_stop_step]["force_norm"][i]
                        ...
                        stats[f"reached@step:{early_stop_step}"].append(
                            int(early_stop_flag)
                        )
                        stats[f"n_iter@step:{early_stop_step}"].append(
                            int(early_stop_n_iter)
                        )
                        stats[f"energy_mse@step:{early_stop_step}"].append(
                            float(energy_mse)
                        )
                        stats[f"force_mse@step:{early_stop_step}"].append(
                            float(force_mse)
                        )
                        stats[f"force_norm@step:{early_stop_step}"].append(
                            float(force_norm)
                        )
                        stats[f"rdkit_energy@step:{early_stop_step}"].append(
                            float(early_stop_energy)
                        )
                        stats[f"pct_of_minimized_energy@step:{early_stop_step}"].append(
                            (initial_energy - early_stop_energy)
                            / (initial_energy - final_energy_rdkit)
                        )
                        ...
                        early_stop_info[early_stop_step]["force_norm"][i] = 0

                    stats["not_finite_action_step"].append(
                        int(not_finite_action_steps[i])
                    )
                    not_finite_action_steps[i] = -1
                    n_iters[i] = 0

                    last_conformation_stats = {
                        key: values[-1] for key, values in stats.items()
                    }
        ...
        if n_conf_processed_total >= next_log_summary:
            print_stats_table(
                n_conf_processed_total,
                aggregate2string(stats, args.pct_min_threshold, args.pct_max_threshold),
            )
            next_log_summary += args.summary_log_interval
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Path to database with initial conformations",
    )
    parser.add_argument(
        "--eval_db_path",
        default="",
