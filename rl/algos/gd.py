...
class GD(object):
    def __init__(self, policy, actor_lr, batch_size=256, actor_clip_value=None, energy_coef=0.01, force_coef=1.):
    ...
    def update(self, replay_buffer, *args):
        ...
        loss = self.force_coef * force_loss + self.energy_coef * energy_loss
        metrics['loss'] = loss.item()
        metrics['energy_loss'] = energy_loss.item()
        metrics['force_loss'] = force_loss.item()
        metrics['energy_loss_contrib'] = energy_loss.item() * self.energy_coef
        metrics['force_loss_contrib'] = force_loss.item() * self.force_coef
