...
class OneStepREDQ(object):
	...
	def update(self, replay_buffer, update_actor, _):
		...
		# --- Critic loss ---
        # First randomly select critics that will be updated
		self.critic.select_critics()

        # Update selected critics
		cur_Q = self.critic(state, action)
		critic_loss = F.mse_loss(cur_Q, reward.expand((-1, cur_Q.shape[1])).unsqueeze(2))
		...
		actor_loss = (alpha * log_pi.squeeze() - self.critic(state, new_action).mean(dim=1)).mean()
